...
class WebhookViewTestCase(WebhookTestCase):
    ...
    def test_no_hmac_is_forbidden(self):
        ...
        self.assertEqual(response.status_code, 401, 'POST orders/create request with no HMAC returns 403 (Forbidden).')
    ...
    def test_invalid_hmac_is_forbidden(self):
        ...
        self.assertEqual(response.status_code, 401, 'POST orders/create request with invalid HMAC returns 403 (Forbidden).')
